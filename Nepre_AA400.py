...
def load_EnergyMatrix400():
    filesum = './Energymatrixs/AAenergymatrixsum3.npy'
    ...
    for m in range(len(EachAA)):
        ...
        for n in range(len(EachAA)):
            if(m == n):
                ...
                radiusSum = radiusDict[EachAA[m].name] + radiusDict[EachAA[n].name] + 3
                if(dis <= radiusSum):
                    #If the distance between two amino acid less than 10, we believe the two amino acid have interaction
                    print matrix[EachAA[m].name][EachAA[n].name]
...
if __name__ == "__main__":
    #args = sys.argv[1:]
    #pdb = args[0]
    pdb = '/Users/xg666/Desktop/loop/getTER/file7/pdb2p4z.ent'
    E = calculate_Energy(pdb)
